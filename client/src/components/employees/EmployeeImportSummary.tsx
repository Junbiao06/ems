...
type EmployeeImportSummaryProps = {
  file: File;
  totalRows: number;
  validRows: number;
  invalidRows: number;
...
export function EmployeeImportSummary({
  file,
  totalRows,
  validRows,
  invalidRows,
}: EmployeeImportSummaryProps) {
  return (
    <div className="grid gap-3 sm:grid-cols-3">
      ...
      <div className="rounded-xl border border-success-border bg-success-surface p-4">
        ...
        <p className="mt-3 text-2xl font-extrabold text-success-text">{validRows}</p>
        <p className="mt-1 text-xs font-semibold text-success-text">Valid rows</p>
      </div>
      <div className="rounded-xl border border-danger-border bg-danger-surface p-4">
        ...
        <p className="mt-3 text-2xl font-extrabold text-danger-text">{invalidRows}</p>
        <p className="mt-1 text-xs font-semibold text-danger-text">Rows with errors</p>
      </div>
    </div>
  );
}
